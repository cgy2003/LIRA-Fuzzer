            ...
            for path in swaggerDoc.Paths do
                ...
                for m in path.Value do
                    ...
                    if not config.ReadOnlyFuzz || readerMethods |> List.contains requestId.method then
                        ...
                        let response: {| bodyResponse: Tree.Tree<LeafProperty,InnerProperty> option; headerResponse: (string * Tree.Tree<LeafProperty,InnerProperty>) list; linkAnnotations: seq<ProducerConsumerAnnotation> |} option = allResponses |> Seq.tryHead
                        ...
                        let responseStr = 
                            match response with
                            | None -> "None"
                            | Some (r: {| bodyResponse: Tree.Tree<LeafProperty,InnerProperty> option; headerResponse: (string * Tree.Tree<LeafProperty,InnerProperty>) list; linkAnnotations: seq<ProducerConsumerAnnotation> |}) -> 
                                sprintf "Some {bodyResponse = %O\n headerResponse = %O\n linkAnnotations = %A}" r.bodyResponse r.headerResponse r.linkAnnotations
                        
                        // let infoStr = sprintf "requestId:%O\nrequestParameters:%O\nresponse:%s\n#############\n" requestId requestParameters responseStr
                        let infoStr = sprintf "requestId:%O\nrequestParameters:%O\nresponse:%O\n#############\n" requestId requestParameters responseStr
                        // LLM Parser
                        File.AppendAllText("E:\\deskbook\\LIRA-Fuzzer\\enhanced\\jellyfin-openapi-10.7.13.txt", infoStr)
                        // LLM Parser End
