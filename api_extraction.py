...
def ask_chatgpt(api_url, request_params):
    ...
    # 生成提示词
    role = (
    "你是一名专门分析 API 规范的 AI 助手。"
    "你的任务是根据 API 的功能和上下文描述，将 API 的 URL 和请求参数分类到预定义的类别中。"
    "你擅长识别与以下类别匹配的关键字：\n"
    "- proxy_api: 与代理或转发请求相关的 API。\n"
    "- upload_api: 与文件或数据上传相关的 API。\n"
    "- path_api: 与路径或路由管理相关的 API。\n"
    "- command_api: 与执行命令或操作相关的 API。\n"
    "- database_api: 与数据库交互相关的 API。\n"
    "- display_api: 与渲染或显示数据相关的 API。\n\n"
    "说明：\n"
    "1. 分析提供的 API URL 和请求参数。\n"
    "2. 识别符合预定义类别的关键字。\n"
    "3. 返回符合以下 JSON 结构的对象：\n"
    "   {\n"
    "       \"ApiParamKeywords\": {\n"
    ...
    "       }\n"
    "   }\n"
    "4. 确保返回的关键字与 API 的功能相匹配。\n"
    "5. 如果未找到匹配的关键字，相应的列表应为空。\n\n"
    "示例：\n"
    "API URL: https://example.com/upload\n"
    "请求参数: [\"file\", \"name\"]\n"
    "输出:\n"
    "{\n"
    "   \"ApiParamKeywords\": {\n"
    ...
    "请分析该 API，并按照指定的 JSON 格式返回相关关键字。\n"
    "⚠ 注意：返回的 JSON 结构必须严格符合以下格式，确保分类准确，避免错误分类或遗漏关键信息。"
)
