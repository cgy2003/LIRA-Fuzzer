...
def convert_swagger2_to_openapi3(spec_location):
    try:
        ...
        if 'swagger' in data and data['swagger'] == "2.0":
            # Form the command
            command = f"api-spec-converter --from=swagger_2 --to=openapi_3 --syntax={syntax} --order=alpha {spec_location} > new_spec.{syntax}"
...
# Use the function
convert_swagger2_to_openapi3("swagger.json")