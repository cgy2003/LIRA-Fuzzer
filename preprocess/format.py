...
def convert_openapi_to_openapi(spec_location, target_version="3.1"):
    try:
        with open(spec_location, 'r',encoding='utf-8') as file:
            if spec_location.endswith('.json'):
                data = json.load(file)
                syntax = 'json'
            elif spec_location.endswith('.yaml',encoding='utf-8') or spec_location.endswith('.yml'):
...
# Use the function
convert_openapi_to_openapi("E:\deskbook\LLM\directus\openapi-main\openapi-main\openapi\merged_openapi_complete.yaml", target_version="3.1")